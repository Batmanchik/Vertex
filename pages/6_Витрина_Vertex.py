...
# ==========================================================================
# 4. Крипто-досье компании
# ==========================================================================
with TABS[3]:
    from apris.crypto_ponzi import visualizations as crypto_viz
    from apris.crypto_ponzi.aggregator import aggregate_transactions
    from apris.crypto_ponzi.explainer import generate_explanation
    from apris.crypto_ponzi.risk_scorer import compute_crypto_ponzi_score
    from apris.crypto_ponzi.tx_generator import PRESET_ORDER, generate_company_transactions

    st.markdown(
        "Второй объект анализа — **юридическое лицо**, которое собирает деньги "
        "с населения и уводит их через криптобиржи. Здесь вся картина одной "
        "компании: с кем она работает, как менялся поток и из чего складывается риск."
    )

    PRESET_LABELS = {
        "SAFE": "Обычная компания",
        "MODERATE": "Есть вопросы",
    ...
    preset = st.radio(
        ...
        horizontal=True,
        help="Переключите профиль — граф, поток и оценка риска пересчитаются целиком.",
    )

    with st.spinner("Генерирую транзакции компании…"):
        dossier = generate_company_transactions(preset=preset, seed=42)
        transactions = dossier["transactions"]
        metrics = aggregate_transactions(transactions)
        verdict = compute_crypto_ponzi_score(metrics)

    head = st.columns(4)
    head[0].metric("Компания", dossier["company_name"])
    head[1].metric("Транзакций", f"{len(transactions):,}".replace(",", " "))
    head[2].metric("Оценка риска", f"{verdict['probability']:.3f}")
    head[3].metric("Уровень", f"{verdict.get('risk_emoji', '')} {verdict['risk_level_ru']}")

    left, right = st.columns([3, 2])
    with left:
        _figure(crypto_viz.plot_counterparty_network(transactions, dossier["company_name"]))
        st.caption(
            "Компания в центре, контрагенты по кругу. Зелёные стрелки — приток, "
            "красные — отток, толщина равна объёму. У пирамиды почти весь отток "
            "уходит на криптобиржи, а приток идёт от физических лиц."
        )
    with right:
        _figure(crypto_viz.plot_inflow_structure_pie(metrics))
        st.caption("Из чего складывается приток: население, бизнес или биржи.")

    _figure(crypto_viz.plot_inflow_outflow_timeline(transactions))
    st.caption(
        "Приток и отток во времени. У пирамиды они идут почти вплотную: деньги "
        "не задерживаются — то же свойство, которое параметр W измеряет на "
        "дропперских кольцах."
    )

    left, right = st.columns([2, 3])
    with left:
        _figure(crypto_viz.plot_factor_contributions(verdict))
    with right:
        st.markdown("#### Из чего собран вердикт")
        st.write(generate_explanation(metrics, verdict, dossier["company_name"]))
        st.dataframe(
            crypto_viz.build_metrics_table(metrics),
            hide_index=True, use_container_width=True,
        )

    st.warning(
        "**Это синтетический демонстрационный сценарий, а не находка системы.** "
        "Компания и её транзакции сгенерированы по выбранному профилю: блок "
        "показывает, как выглядит разбор на уровне юридического лица, но ничего "
        "не обнаруживает. Граф при этом построен из транзакций, а не из вердикта — "
        "оценка на него не влияет. Всё остальное на витрине работает на уровне "
        "счетов и групп и является результатом детектора."
    )
