...
selected_id = st.selectbox(
    "Дело на проверку",
    order,
    # «40 счетов» читалось как «сорок счетов одного человека». Кандидат —
    # это одно дело, а счета внутри него состав, поэтому единица названа
    # первой, а размер идёт после неё как характеристика.
    format_func=lambda cid: (
        f"Дело {cid} · в составе {by_id[cid].size} счетов, "
...
st.caption(
    f"**{selected_id} — одно дело и одна единица анализа**: система предложила "
    "эти счета как связанные и оценивает их вместе, одной оценкой. Связь найдена "
    "по общим ресурсам (" + ", ".join(candidate.link_reasons) + "). "
    "Счета внутри принадлежат разным владельцам, и в этом смысл группировки: "
    "по отдельности каждый выглядит обычно, вместе складываются в схему. "
