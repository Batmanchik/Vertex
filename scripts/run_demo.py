...
1. собирает очередь аналитика (`scripts/run_pipeline.py`), чтобы страница
   «Очередь аналитика» открывалась с данными, а не с инструкцией, и
   пересобирает витрину (`scripts/make_site.py`) на свежих числах;
...
def main() -> int:
    ...
    # Витрина — статический файл, и собрать его дешевле, чем объяснять на
    # защите, почему на странице вчерашние числа.
    site = subprocess.run([sys.executable, "scripts/make_site.py"], env=environment, check=False)
    if site.returncode != 0:
        print("     витрина не пересобралась — открывайте прежний artifacts/site/index.html")
